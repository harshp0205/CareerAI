...
class VideoInterviewService {
  ...
  async generateQuestions({ 
    ...
    const questionTypeInstructions = {
      'detailed': 'Comprehensive questions requiring 2-3 minute responses with examples and explanations',
      'short': 'Concise questions requiring 30-60 second focused answers',
      'one-liner': 'Quick-fire questions requiring brief 10-20 second responses or single sentences'
    };

    const typeDistribution = this.distributeQuestionTypes(questionCount, questionTypes);
    ...
    const prompt = `
      ...
      Question type distribution: ${JSON.stringify(typeDistribution)}
      Analysis level: ${analysisLevel}
      
      Question Types Instructions:
      ${questionTypes.map(type => `- ${type}: ${questionTypeInstructions[type]}`).join('\n')}
      
      For each question, provide:
      1. The question text
      2. Category (behavioral, technical, or situational)
      3. Question type (detailed, short, or one-liner)
      4. Expected answer key points
      5. Time limit recommendation (10-300 seconds based on type)
      6. Evaluation criteria
      7. Sample excellent answer outline
        ...
        "questions": [
          {
            ...
            "sampleAnswerOutline": "Brief outline of an excellent answer"
          }
        ]
      }
      
      Make questions relevant to ${industry} and appropriate for ${difficulty} level candidates.
      Ensure a good mix of the requested categories and types.
      
      Question Type Guidelines:
      - Detailed (2-3 min): Complex scenarios, past experiences with STAR method
      - Short (30-60 sec): Direct questions about skills, preferences, or quick problem-solving
      - One-liner (10-20 sec): Quick facts, yes/no with brief explanation, rapid-fire technical terms
    `;
  ...
  distributeQuestionTypes(totalQuestions, requestedTypes) {
    const distribution = {};
    
    if (requestedTypes.length === 1) {
      distribution[requestedTypes[0]] = totalQuestions;
      return distribution;
    }
    
    // For multiple types, distribute evenly with some logic
    const basePerType = Math.floor(totalQuestions / requestedTypes.length);
    const remainder = totalQuestions % requestedTypes.length;
    
    requestedTypes.forEach((type, index) => {
      distribution[type] = basePerType + (index < remainder ? 1 : 0);
    });
